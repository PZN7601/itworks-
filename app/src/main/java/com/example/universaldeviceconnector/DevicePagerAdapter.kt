...
class DevicePagerAdapter(
    ...
    private fun setupNetworkView() {
        ...
        val topologyBtn = networkView.findViewById<Button>(R.id.topologyButton)
        topologyBtn.visibility = View.VISIBLE
        topologyBtn.setOnClickListener {
            if (networkDevices.isNotEmpty()) {
                val intent = Intent(activity, NetworkTopologyActivity::class.java)
                intent.putExtra("devices", ArrayList(networkDevices))
                activity.startActivity(intent)
            } else {
                Toast.makeText(activity, "No network devices found. Run a scan first.", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun runBluetoothDiagnosis(device: BluetoothDevice) {
        ...
        val bluetoothAdapter = activity.bluetoothAdapter
        val paired = bluetoothAdapter?.bondedDevices?.contains(device) ?: false
    ...
    private fun runNetworkDiagnosis(device: MainActivity.DeviceInfo) {
        ...
        ioScope.launch {
            ...
            val serviceMap = mapOf(
                ...
                80 to "HTTP", 443 to "HTTPS", 515 to "LPD (Printer)", 631 to "IPP (Printer)",
                3389 to "RDP", 8080 to "HTTP-Alt", 8443 to "HTTPS-Alt"
            )
